...
## Notes / assumptions encoded in the model

Key constants and assumptions are hard-coded in `ms_lions.py` (effective population sizes, mutation rate, recombination rate, generation time, and split times). The divergence time between modern and cave lion lineages is explored across a range defined in the script.
